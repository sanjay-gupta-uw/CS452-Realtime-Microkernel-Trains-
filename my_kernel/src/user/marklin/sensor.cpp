...
void SensorManager::ReadAll(int num_banks, RingBuffer<int> *recent_sensors) {
    if (num_banks < 1 || num_banks > NUM_BANKS) num_banks = NUM_BANKS;
    
    int ret = MARKLIN_IO_SERVER::Putc(marklin_io_tid, MARKLIN, READ_ALL_SENSOR_BASE + num_banks);
    if (ret != 0) {
        return;
    }

    clock_delay(20); //Fix this part

    for (int bank = 0; bank < num_banks; ++bank) {
        uint8_t byte1 = MARKLIN_IO_SERVER::Getc(marklin_io_tid, MARKLIN);
        uint8_t byte2 = MARKLIN_IO_SERVER::Getc(marklin_io_tid, MARKLIN);
        processSensorData(bank, byte1, byte2, recent_sensors);
    }
}
...
void SensorManager::Reset(bool reset_on) {
    int ret = MARKLIN_IO_SERVER::Putc(marklin_io_tid, MARKLIN, reset_on ? RESET_MODE_ON : RESET_MODE_OFF);
    clock_delay(20);
}
