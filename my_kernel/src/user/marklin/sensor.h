...
public:
    ...
    void ReadAll(int num_banks, RingBuffer *recent_sensors);
    void Reset(bool reset_on);
    void InitDisplay(IO *io, int location);
    void Display(IO *io, int location, RingBuffer *recent_sensors);
...
private:
    ...
    void processSensorData(int bank, uint8_t byte1, uint8_t byte2, RingBuffer *recent_sensors);
};
