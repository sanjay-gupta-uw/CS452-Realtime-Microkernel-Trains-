...
#include <string.h>
// #include "../include/track_node.h" // included in track_data.h so redundant
...
static bool isNodeInLoop(track_node *node)
{
    if (node->is_node_in_loop)
        return true;
    return false;
}
...
void Track::initialize_loop()
{
    ...
    {
        SensorNode node;
        loop_sensors.Pop(&node);
        int bank = node.bank[0] - 'A';
        int idx = bank * 16 + node.num - 1;
...
{
    // uart_printf(CONSOLE, "Looking for node: %s\r\n", name);
    for (int i = 0; i < TRACK_MAX; i++)
    {
        if (track[i].name && strcmp(track[i].name, name) == 0)
        {
            // uart_printf(CONSOLE, "Found node %s at index %d\r\n", name, i);
            return &track[i];
        }
    }
    // uart_printf(CONSOLE, "Node %s not found!\r\n", name);
    return NULL;
}
